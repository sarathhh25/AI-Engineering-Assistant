  ...
  return (
    <div
      className={`py-5 px-4 sm:px-8 flex gap-4 transition-colors ${
        isUser
          ? 'bg-transparent'
          : 'bg-muted/30 dark:bg-zinc-900/30 border-y border-border/40'
      }`}
    >
      {/* Avatar */}
      ...
      <div className="flex-1 min-w-0 space-y-3 text-xs leading-relaxed text-foreground">
        ...
        {/* Text Paragraphs */}
        <div className="space-y-2.5 text-foreground/90 text-[13px] leading-relaxed">
          {message.content.split('\n\n').map((paragraph, pIdx) => {
            ...
            if (paragraph.startsWith('- ') || paragraph.startsWith('1. ') || paragraph.startsWith('2. ') || paragraph.startsWith('3. ')) {
              ...
              return (
                <ul key={pIdx} className="list-disc pl-4 space-y-1 text-foreground/90">
        ...
        {message.references && message.references.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 pt-1">
            ...
            {message.references.map((ref) => (
              <span
                key={ref}
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-secondary border border-border text-[10px] font-mono text-primary cursor-pointer hover:border-primary/40 transition-all"
              >
        ...
        {message.tableData && (
          <div className="my-3 rounded-lg border border-border overflow-x-auto bg-card font-mono text-[11px]">
