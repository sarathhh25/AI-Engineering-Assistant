...
const ACTIONS = [
  ...
  },
  {
    ...
    icon: Sparkles,
    color: 'from-blue-600/20 to-indigo-600/20 text-primary',
  ...
  return (
    <div className="space-y-3 relative select-none">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-[10px] text-muted-foreground font-mono">2x2 Workspace Grid</span>
      </div>
      ...
      {activeRunningAction && (
        ...
        >
          <div
            ...
            className="w-full max-w-lg bg-card border border-border rounded-2xl shadow-2xl p-5 space-y-4"
          >
            <div className="flex items-center justify-between border-b border-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-semibold text-foreground text-sm">{activeRunningAction.title}</h3>
                  <p className="text-[10px] text-muted-foreground">Executing Agent Task (-50 AI Credits)</p>
                </div>
              </div>
            ...
            {isRunning ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-1.5 text-emerald-500 font-semibold font-mono text-xs">
                  <CheckCircle2 className="size-4" />
                  <span>Execution Complete</span>
                </div>

                <div className="p-3.5 rounded-xl bg-muted/40 border border-border font-mono text-[11px] text-foreground leading-relaxed whitespace-pre-wrap">
                  {resultText}
                </div>
                ...
                <div className="pt-2 flex justify-end">
                  <button
                    onClick={() => setActiveRunningAction(null)}
                    className="px-4 py-1.5 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-xs transition-colors cursor-pointer shadow-sm"
                  >
                    Done
                  </button>
                </div>
